...
<aside class="main-sidebar">
    <section class="sidebar">
        ...
        <ul class="sidebar-menu">
			@foreach($menuSubMenuArr as $menuVal)
               @if(is_array($menuVal['submenus']) && count($menuVal['submenus']) > 0)							
                    @if(isset($roleMenuArrArr['editMenuList']) && is_array($roleMenuArrArr['editMenuList']) && in_array($menuVal['_id'],$roleMenuArrArr['editMenuList']))
                    ...
                    <li class="treeview @if($menuVal['controller'] == $controller) active @endif">
                        ...
                        <ul class="treeview-menu">
                            @foreach($menuVal['submenus'] as $subMenuVal)
                                @if(isset($roleMenuArrArr['editSubMenuList']) && is_array($roleMenuArrArr['editSubMenuList']) && in_array($subMenuVal['_id'],$roleMenuArrArr['editSubMenuList']))

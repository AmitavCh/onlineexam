                            ...
                            <div class="row">
                                <div class="col-md-3 margine10bot"> 
                                    <div class="form-group">
                                        <input type="text" name="search" maxlength="25" class="form-control" id="{{Request::get('search')}}" placeholder="Search by role name" autocomplete="off"/>
                                    </div>
                                </div>
                            ...
                            <table class="table table-bordered table-striped">
                                ...
                                <tr>
                                    ...
                                    <td class="text-center">							
                                        <a class="iframeD btn-sm" title="Edit Record" href="{{ URL::to('/master/add_role_data/'.base64_encode(base64_encode($post->id))) }}">
                                            <i class="fa fa-edit"></i>
                                        </a>
                                        @if($post->is_active == 'N')
                                        <a class="btn-sm" title="Active Record" href="{{URL::to('master/roleActive/'.base64_encode(base64_encode($post->_id)))}}" onclick="return confirm('Are you sure to Active Record ?')">
                                            <i class="fa fa-check-circle-o"></i>
                                        </a> 
                                        @endif
                                        @if($post->is_active == 'Y')
                                        <a class="btn-sm" title="Deactive Record" href="{{URL::to('master/roleDeactive/'.base64_encode(base64_encode($post->_id)))}}" onclick="return confirm('Are you sure to De-Active Record ?')">
